...
# 	print(np.unique(pr),'-----------np.unique(pr)-----------------------',pr.shape,'\n')
...
# 	print('------np.unique(seg_img) before resize -----------------',np.unique(seg_img),'  shape: ',seg_img.shape,'\n')
	seg_img = cv2.resize(seg_img  , (orininal_w , orininal_h ))
# 	print('------np.unique(seg_img)after resize -----------------',np.unique(seg_img),' shape: ',seg_img.shape,'\n')
	if not out_fname is None:
		print(out_fname)

		d="/home/tookai-2/Desktop/code1/image-segmentation-keras/out/"

		cv2.imwrite(d+out_fname , seg_img )
...
def evaluate( model=None , inp_inmges=None , annotations=None , checkpoints_path='./checkpoints/_config.json' ):
	print(model,inp_inmges,annotations,checkpoints_path)
